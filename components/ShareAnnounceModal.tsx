...
// for RETURNING players (anyone with prior deadlockle state) — they
// played without sharing, so "you can now share" is news to them;
// brand-new players discover the Share button organically on their
// first result and never see this. Mounted site-wide from the root
// layout so it greets returning players on whatever page they land on.
//
// Ported from OWdle's ShareAnnounceModal — keep the repos in lockstep.

const SEEN_KEY = "deadlockle.announce.shareLinks";
...
const ANNOUNCE_EXPIRED =
  Date.now() > new Date("2026-06-20T00:00:00-07:00").getTime();

// Fixed example card: a daily 5/5 sweep (date 2026-06-06, results
// 3/2/4/3/2, no hints, no hard mode). Hardcoding the code keeps this
// component pure — encoding live results would need today's date in
// render — and the image is immutable + permanently edge-cached anyway.
// It renders with the CURRENT card design since the OG worker draws it
// live.
const EXAMPLE_CODE = "260606-32432-00";
...
function shouldAnnounce(): boolean {
  ...
  try {
    ...
    // Returning-player heuristic: any prior deadlockle state at all.
    for (let i = 0; i < window.localStorage.length; i++) {
      const k = window.localStorage.key(i) ?? "";
      if (k.startsWith("deadlockle") && k !== SEEN_KEY) return true;
    }
    return false;
  } catch {
    return false;
  ...
  const overlay = (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-[420px] max-h-[92vh] overflow-auto rounded-(--radius-card) border border-line bg-surface text-ink"
      >
        <div className="flex items-center justify-between border-b border-line px-4 py-3">
          <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-info">
            <span className="mr-2 rounded-(--radius-pill) bg-info px-2 py-0.5 text-on-info">
              New
            </span>
            Share your results
          </p>
        ...
        <div className="flex flex-col gap-4 px-5 py-5">
          ...
          <button
            type="button"
            onClick={dismiss}
            className="inline-flex w-full items-center justify-center rounded-(--radius-pill) bg-info px-5 py-3 font-mono text-[12px] uppercase tracking-[0.22em] text-on-info transition-all hover:brightness-110 active:scale-[0.99]"
          >
            Got it
          </button>
        </div>
