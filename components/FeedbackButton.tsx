...
// D1; the `source` column tags each row with 'deadlockle' so the inbox
// can be filtered per site.
//
...
export function FeedbackButton() {
  ...
  const openDialog = () => {
    setOpen(true);
    // Once the player taps to open the dialog they've engaged with the
    // popup, so collapse the sticky footer immediately rather than
    // leaving it lingering behind the modal.
    ...
    <>
      ...
      <button
        ...
        className={clsx(
          ...
          allDone
            ? "border-2 border-correct bg-correct/15 px-5 py-3 font-mono text-xs uppercase tracking-[0.22em] text-correct shadow-[0_0_28px_-6px_rgba(120,210,150,0.55)] hover:bg-correct/25"
      ...
      {popupActive ? (
        <div
          role="status"
          aria-live="polite"
          className="fixed inset-x-0 bottom-0 z-40 border-t border-correct/60 bg-surface/95 px-4 py-3 shadow-[0_-6px_24px_rgba(0,0,0,0.55)] backdrop-blur-sm transition-transform duration-300 md:hidden"
        ...
        >
          ...
          {/* Local keyframes so we don't need to touch globals.css for a
              one-off entrance animation. */}
