...
// unfurls into the personalized result card wherever it lands (Discord,
...
//
// Ported from OWdle's ShareButton — keep the two repos in lockstep.
// (OWdle's renderCard fallback path for no-OG surfaces is deliberately
// absent here: every Deadlockle share surface has a server-rendered
// unfurl image, so the client-capture dependency never ships.)

type Props = {
  // Bare share link — goes on the clipboard / into the OS sheet as-is.
  url: string;
  // The /og/r/[code] image matching `url`. The modal previews it
  // (truthful by construction — it IS what friends see).
  ogImageUrl: string;
  filename: string;
...
export function ShareButton({
  ...
  // Both variants are SOLID fills, square per the parlour pill radius.
  // Primary takes the info teal (share = info family, matching the
  // text-share block); soft is a muted chip for quieter hosts.
  const btnClass =
    variant === "primary"
      ? "group inline-flex items-center gap-2 rounded-(--radius-pill) bg-info px-5 py-3 text-on-info shadow-lg shadow-black/30 transition-all duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] hover:brightness-110 hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98] disabled:opacity-50"
      : "group inline-flex items-center gap-2 rounded-(--radius-pill) border border-line bg-muted px-4 py-2 text-ink transition-all hover:border-info hover:text-info active:scale-[0.98] disabled:opacity-50";
    ...
    <>
      ...
      {nativeCapable && (
        <button
          ...
          className="inline-flex items-center justify-center rounded-(--radius-pill) border border-line bg-muted p-3 text-ink-soft transition-all hover:border-info hover:text-info active:scale-[0.98]"
        >
          <ImageGlyph />
        </button>
      )}
...
// server-side), so it must shoulder transient cold-isolate 503s — a
// give-up-on-first-error prefetch left the link un-warmed exactly when
// warming mattered most. Each retry fetches a DISTINCT URL
...
// are per-player unique (never prewarmed), so they always run this
// gauntlet; four attempts at ~50% cold-kill odds leave a code
// un-warmed ~6% of the time, and the modal's own ladder picks up from
// there.
