...
import { archiveFillStatus, archiveWindow, type DayFill } from "@/lib/archive";
...
//
//   won   → solid green + ✓   (the reward; a green run IS the "streak")
//   lost  → solid far-red + ✕ (present but redeemable: replaying + winning
//           flips it green via the best-outcome union)
//   none  → empty recessed slot
//   today → accent ring; links to the LIVE /<mode> (played live, not here)

// Modes with a live archive route (/archive/<slug>). Widen as each mode's
// archive ships. The grid + cells are otherwise mode-agnostic — the slug
// only drives the fill lookup and the replay hrefs.
export type ArchiveModeSlug = "classic" | "mugshot" | "sound";
...
// Pre-mount placeholder: the same window rendered as empty cells so the
// strip's dimensions are stable before localStorage fills the statuses.
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Quiet tally — reinforces the "fill the week" goal without a streak
          number (the green run is the visual streak). Flips to a
          celebration when every day in the window is green. */}
      <p className="text-center font-mono text-[11px] uppercase tracking-[0.16em]">
        {!mounted ? (
...
function ArchiveCell({
  ...
  const fillClass =
    outcome === "won"
      ...
      : outcome === "lost"
        ? "bg-far text-on-far"
        : inProgress
          ? "border border-accent bg-inset text-ink"
  ...
  return (
    ...
    >
      <span className="font-mono text-[9px] uppercase tracking-[0.14em] text-ink-faint sm:text-[10px]">
        {weekday}
      </span>
      <motion.span
        ...
        className={`relative flex aspect-square w-full items-center justify-center rounded-(--radius-card) transition-transform duration-150 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.05] group-focus-visible:scale-[1.05] ${fillClass}${ring}`}
      >
        {glyph}
        ...
        {isToday && outcome === "none" && (
          <span
            aria-hidden
            className="font-mono text-[8px] uppercase tracking-[0.12em] text-accent sm:text-[9px]"
          >
            Today
          </span>
        )}
