// Tip-jar panel. Sits centered on the homepage on its own light "paper"
// card (see HomeContent) so the one personal/human panel pops off the dark
// board — the vote widget it used to pair with moved to /whats-next. Stacks
// vertically: heading + copy on top, Ko-fi profile preview + tip button +
// share link below. The Ko-fi action opens an in-page modal containing
// Ko-fi's official panel iframe — see KofiModal.
//
// Because the card is light, text here is dark-on-light: `text-canvas`
// (the dark base color) for strong text, plus fixed on-light hexes for the
// muted slate + link/hover tiers, mirroring how the sister-site cards
// hardcode their own on-light palettes.
import { SUPPORT_LINKS, SHARE_TEXT, SUPPORT_SITE_URL } from "@/lib/support";
import { KofiModal } from "./KofiModal";

const TWITTER_INTENT = `https://twitter.com/intent/tweet?text=${encodeURIComponent(
  SHARE_TEXT,
)}&url=${encodeURIComponent(SUPPORT_SITE_URL)}`;

export function SupportLinks() {
  return (
    <div className="flex h-full flex-col justify-center">
      <h3 className="text-center font-display text-2xl text-canvas sm:text-3xl">
        Support me :D
      </h3>
      <p className="mt-2 text-center text-sm text-[#4a5b74]">
        Daily puzzles take time. If Deadlockle made your day, slide a coffee my way {'( ๑‾̀◡‾́)σ"'}
      </p>
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="group relative shrink-0">
            ...
            {/* Speech bubble — pops in on avatar hover / keyboard focus.
                ...
                avatar from the bubble's bottom-left corner. Inverted to a
                dark bg-canvas bubble with cream text now that the card is
                light, so it stands out against the paper. pointer-events-none
                so the cursor passing up onto the bubble doesn't strand the
                avatar's hover state. */}
            <div
              role="tooltip"
              className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-3 w-48 origin-bottom-left scale-90 rounded-2xl border border-line bg-canvas px-3 py-2 text-center font-sans text-xs leading-snug text-ink opacity-0 shadow-[0_6px_16px_-4px_rgba(0,0,0,0.45)] transition-all duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-100 group-hover:opacity-100 group-focus-within:scale-100 group-focus-within:opacity-100"
            >
          ...
          <div>
            <p className="font-display text-2xl text-canvas leading-none">
              yush
            </p>
