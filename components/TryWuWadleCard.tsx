...
// Wuthering Waves quiz). Same shape and variants as TryOWdleCard — keep the
// two in lockstep when either changes. Per the network convention a
// destination is always its own accent-on-dark: "Play WuWadle" is misty
...
//     OWdle card in the home sister row. Sharp corners + flat-at-rest to
//     match Deadlockle's editorial chrome; the accent border carries the edge.
//   - compact — legacy slim layout, retained for sibling API parity.
const WUWADLE_URL =
  "https://wuwadle.app/?utm_source=deadlockle&utm_medium=sister-site";
...
export function TryWuWadleCard({ compact = false }: { compact?: boolean }) {
  if (compact) {
    return (
      ...
      >
        {/* Faint corner ornament — hint of WuWa misty-blue so the swap reads */}
        <span
          aria-hidden
          className="pointer-events-none absolute inset-y-0 right-0 w-1/2"
          style={{
            background:
              "linear-gradient(110deg, transparent 0%, rgba(95,159,214,0.06) 70%, rgba(95,159,214,0.16) 100%)",
          }}
        />
        <div className="relative flex flex-col gap-3">
          <div className="flex-1">
            ...
            <p className="mt-1 text-xs text-ink-soft">
              Same daily format, different game.
            </p>
          </div>
