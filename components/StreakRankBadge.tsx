...
const BEST_KEY = "deadlockle.streakRank.best";
...
// when no tier is earned (rendering the plain flame), so the watcher keeps
// running.
...
// until mobile placement is decided. Deadlock badge art is wide
// (906×584), so the img box is landscape where OWdle's was square.
export function StreakRankPill({
  ...
  // Pill chrome follows the tier accent (emerald / gold / silver), not the
  // site amber — the rank's color identity should read from the header at
  // a glance. Exposed as a CSS var so Tailwind's opacity modifiers work
  // against the per-tier hex.
  const accent = STREAK_TIER_ACCENT[tier];
  return (
    <button
      type="button"
      onClick={onClick}
      title={`${streak}-day streak · Streak Rank: ${label}`}
      aria-label={`${streak} day streak. Streak rank: ${label}.`}
      style={{ "--tier": accent } as React.CSSProperties}
      className="hidden items-center gap-2 rounded-(--radius-pill) border border-(--tier)/40 bg-(--tier)/5 px-2.5 py-1 text-(--tier) transition-colors hover:border-(--tier) hover:bg-(--tier)/10 sm:inline-flex"
    >
      <FlamePip size={16} />
      <span className="font-sans text-sm font-bold tabular-nums leading-none">
        {streak}
      </span>
      <span aria-hidden className="h-3.5 w-px bg-(--tier)/30" />
