...
// :8798 (port kept in sync there — OWdle's stack owns :8799 and both
// run simultaneously) and the preview renders the real card instead of
// falling back.
//
...
import { encodeResults, encodeRoundResult } from "./shareUrl";

const OG_ORIGIN =
  process.env.NODE_ENV === "development" ? "http://localhost:8798" : "";
...
// design-tweaking. A fresh module evaluation → fresh query param →
// the cache can't match. Prefetch/modal parity holds because both
// read the same module constant within a page session.
const DEV_BUST =
