...
// the stats line surfaces mid-morning rather than mid-evening at the
// current Deadlockle DAU. The cost is wider noise at low samples, but
// the stat is approximate by design (5-min cache + distinct_id dedup),
// so cheap surfacing wins over precision. Result cards suppress the
// line when the relevant bucket is below threshold so under-sampled
// days look identical to pre-Phase-5 builds.
...
export type StatsMode = "classic" | "ability" | "mugshot" | "sound" | "item";
...
// implicitly on full page reload, which is also when the 5-min server
// cache would have rolled the underlying response anyway.
