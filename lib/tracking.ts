...
// day by stashing a marker in localStorage. hint_used fires from
...
export function trackModeCompleted(opts: {
  ...
  // encoding (hero/item key; Sound entries are "<heroKey>@<target>").
  // Correctness stays derivable: outcome "won" means the final element
  // landed. Classic hint spends are NOT in here — hints_used counts
  // them and hint_used carries their positions.
  guessIds: string[];
...
// mobile_imps_total, plus doc_scroll_h / max_scroll_px). The two are
// mutually exclusive per pageview. Event + prop names are OWdle-identical
// (shared DailyDles dashboards).
