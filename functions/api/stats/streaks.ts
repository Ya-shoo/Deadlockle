...
// StreakRankBadge to map a player's current streak to a Deadlock rank
// tier (Eternus / Ascendant / Phantom). Ported from OWdle; keep in
// lockstep.
//
// Pool: distinct_ids who fired `daily_completed` (site = 'deadlockle')
// in the last 30 days, each taken at their latest reported
// `streak_current` (argMax by timestamp). Ranked by that value. This is
// a ROLLING window, not a single Pacific puzzle day, so it lives apart
// from stats/today.ts.
//
//   cutoffs.eternus   = streak at the 99th percentile (top-1% threshold)
//   cutoffs.ascendant = 95th percentile (top 5%)
//   cutoffs.phantom   = 90th percentile (top 10%)
//
...
type StreakCutoffs = { eternus: number; ascendant: number; phantom: number };
...
async function fetchFromPostHog(env: Env): Promise<StreaksResponse> {
  ...
  const query = [
    ...
    "    AND properties.site = 'deadlockle'",
    "    AND timestamp > now() - INTERVAL 30 DAY",
    "  GROUP BY distinct_id",
    ")",
    "SELECT",
    "  count() AS n,",
    "  quantile(0.99)(cur) AS c_eternus,",
    "  quantile(0.95)(cur) AS c_ascendant,",
    "  quantile(0.90)(cur) AS c_phantom",
    "FROM per_user",
  ].join("\n");
  ...
  return {
    n,
    cutoffs: {
      eternus: Number(row[1]) || 0,
      ascendant: Number(row[2]) || 0,
      phantom: Number(row[3]) || 0,
    },
  };
}
