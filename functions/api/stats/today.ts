...
// filtered to `properties.site = 'deadlockle'` so OWdle's events don't
...
type Mode = "classic" | "ability" | "mugshot" | "sound" | "item";
const MODES: Mode[] = ["classic", "ability", "mugshot", "sound", "item"];
...
export const onRequestGet: Handler = async ({ request, env }) => {
  ...
  // screens just look identical to pre-Phase-5 builds.
...
function emptyModes(): Record<Mode, ModeBucket> {
  return {
    ...
    mugshot: { won: 0, lost: 0, gaveUp: 0, total: 0 },
    sound: { won: 0, lost: 0, gaveUp: 0, total: 0 },
    item: { won: 0, lost: 0, gaveUp: 0, total: 0 },
  };
}
