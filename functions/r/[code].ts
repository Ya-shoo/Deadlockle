...
const SITE_ORIGIN = "https://deadlockle.com";
...
<meta property="og:site_name" content="Deadlockle" />
...
<style>
body{background:#0c1820;color:#f3e8d3;font-family:system-ui,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}
a{color:#d6a05c}
</style>
</head>
<body>
<a href="${escapeHtml(redirectUrl)}">Continue to Deadlockle →</a>
</body>
</html>`;
}
...
// logs instead of sending, same split as the OG renderer's cache
// policy. Counts are directional: the 5-minute s-maxage above means an
// edge-cached repeat fetch of the same code never reaches this
// function, and platforms with their own embed caches (Discord) scrape
// once per URL anyway. The cron pre-warm hits /og/r/ directly, not
// this shell, so it never inflates these numbers.
...
export const onRequestGet: Handler = async ({ params, request, waitUntil }) => {
  ...
  if (daily) {
    ...
    const title =
      wonCount === total
        ? `Deadlockle — swept all ${total} modes in ${totalGuesses} guesses`
        : `Deadlockle — ${wonCount}/${total} modes in ${totalGuesses} guesses`;
    const description = `Deadlockle daily for ${dateLabel}. Click to play today's puzzles.`;
  ...
  if (round) {
    ...
    const title =
      round.outcome === "won"
        ? `Deadlockle ${label} — solved in ${round.guesses} ${
            round.guesses === 1 ? "guess" : "guesses"
          }`
        : `Deadlockle ${label} — missed it`;
    const description = `Deadlockle ${label} for ${dateLabel}. ${
